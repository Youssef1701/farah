...
$(document).ready(function () {
  ...
  function createOverlay() {
    ...
    $.ajax({
      type: "PUT",
      url: "/message",
      data: { messageId, completedMeditation: true },
    });
  }
  function resizeStar() {
  ...
  function initializeDisappear() {
    ...
    if (!isPreview && !startedMeditation) {
      const data = { message: `${allText}`, xp, uuid, meditationId };
      window
        ...
        .then((geo) => {
          $.ajax({
            type: "POST",
            url: "/message",
            data: { ...data, zip: geo.zip_code, country: geo.country_code },
          }).done(function (msgId) {
            messageId = msgId;
          });
        })
        .catch(function (error) {
          $.ajax({ type: "POST", url: "/message", data: { ...data } }).done(
            function (msgId) {
              messageId = msgId;
            }
          );
  ...
  $(".cta").on("click", function () {
    ...
    $.ajax({
      type: "PUT",
      url: "/message",
      data: { messageId, ctaClicked: true },
    });
  });
});
